...
# ──────────────────────────────────────────────────────────────────
#
# Both were inline-defined here until Phase-29 (2026-05-30); they now
# live in dedicated router files for maintainability. The behaviour,
# route paths, and request/response payloads are unchanged.

try:
    from routers.sheet_sync import (
        sheet_sync_router as _sheet_sync_router,
        init              as _init_sheet_sync_router,
    )
    _init_sheet_sync_router()
    app.include_router(_sheet_sync_router)
except Exception as _ss_exc:
    logger.exception(f"Failed to mount sheet-sync router: {_ss_exc}")

try:
    from routers.analytics import (
        analytics_router as _analytics_router,
        init             as _init_analytics_router,
    )
    _init_analytics_router()
    app.include_router(_analytics_router)
except Exception as _an_exc:
    logger.exception(f"Failed to mount analytics router: {_an_exc}")
