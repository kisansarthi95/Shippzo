...
Auth model:
  • All /api/support/* endpoints require login (Depends current_user).
  • /api/admin/support/* endpoints require is_admin (Depends current_user
    + 403 check inline so the route declaration stays simple).
"""
...
# Late-bound at init() so we don't circular-import server.py at module
# load time. Same pattern as the other Phase-1 modular routers.
db = None
...
# ────────────────────────────────────────────────────────────────────
# Router (mounted under /api below)
# ────────────────────────────────────────────────────────────────────
...
async def create_ticket(
    ...
    current_user: Dict[str, Any] = Depends(lambda: get_current_user()),
):
    """Create a new support ticket. The author's first message is
    derived from `description` and stored inline in `messages[0]` so
    the detail view doesn't need a second round-trip on first load."""
...
async def list_my_tickets(
    ...
    """List the current user's tickets newest-first. Optional `status`
    ...
    async for t in cursor:
        # Strip the messages array on list view — the detail endpoint
        # returns full thread. Keep only the count + last preview so
        # the list UI can show "3 replies · last: …" badges.
...
# ────────────────────────────────────────────────────────────────────
# Late-binding init — call after server.py finishes defining `db`
# and `get_current_user`.
# ────────────────────────────────────────────────────────────────────
def init():
    global db, get_current_user
    from server import db as _db, get_current_user as _gcu
    db = _db
