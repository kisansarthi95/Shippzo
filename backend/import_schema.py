...
SCHEMA_FIELDS: List[str] = [
    ...
    "order_id",
    "notes",
    # Phase-32 (2026-06) — Promote tracking_id + master_order_id to
    # first-class schema fields so users can map them in their Sheet
    # column-mapping. The dedup logic in server._enrich_pending_orders
    # uses these to flag a sheet row as `already_shipped` whenever the
    # row's tracking_id OR master_order_id matches an existing Shipment.
    # Without explicit aliases, those reads returned "" and shipped
    # rows kept reappearing as ghost pendings.
    "tracking_id",
    "master_order_id",
...
HEADER_ALIASES: Dict[str, str] = {
    ...
    "product":         "items",
    "products":        "items",
    # Phase-32 — Tracking ID aliases. Covers the major Indian courier
    # acronyms (AWB, LR, docket, consignment) so a user can paste any
    # of these column headers and we resolve them all to `tracking_id`.
    ...
    "consignment":      "tracking_id",
    "consignment_no":   "tracking_id",
    "consignment_number": "tracking_id",
    "lr":               "tracking_id",
    "lr_no":            "tracking_id",
    "lr_number":        "tracking_id",
    "docket":           "tracking_id",
    "docket_no":        "tracking_id",
    "docket_number":    "tracking_id",
    "shipment_id":      "tracking_id",
    "ship_id":          "tracking_id",
    "way_bill":         "tracking_id",
    "waybill":          "tracking_id",
    # Phase-32 — Master Order ID aliases. The internal canonical ID
    # generated by Shippzo (vs. order_id which is the operator's
    # external/business order ref). Adding these lets the dedup
    # logic detect re-imports of already-shipped rows.
    "master_order_id":  "master_order_id",
    "master_order":     "master_order_id",
    "master_id":        "master_order_id",
    "internal_id":      "master_order_id",
    "internal_order_id": "master_order_id",
    "system_id":        "master_order_id",
    "system_order_id":  "master_order_id",
    "mo_id":            "master_order_id",
    "shipment_master_id": "master_order_id",
